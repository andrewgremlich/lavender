...
export const api = {
  ...
  metrics: {
    getAll: () =>
      request<Array<{ id: string; encryptedData: string; iv: string; createdAt: string; expiresAt: string }>>('/metrics'),
  ...
  passkeys: {
    getRegistrationOptions: () =>
      request<any>('/passkeys/register/options', { method: 'POST' }),
    ...
    verifyAuthentication: (response: any, challenge: string) =>
      request<{ token: string; username: string }>('/passkeys/authenticate/verify', {
        method: 'POST',
        body: JSON.stringify({ response, challenge }),
      }),
    list: () =>
      request<Array<{ id: string; credential_id: string; created_at: string }>>('/passkeys'),
    delete: (id: string) =>
