...
function setStatus(next: SyncStatus) {
	...
	window.dispatchEvent(
		new CustomEvent("sync-status-change", { detail: { status } }),
	);
}
...
export async function refreshFromServer(): Promise<EncryptedEntry[]> {
	try {
		const serverEntries = await api.metrics.getAll();

		// Replace local cache with server state (server is authoritative)
		...
		// Offline or server error — return whatever is cached
...
export async function flush(): Promise<void> {
	...
	try {
		...
		// Detect and cancel paired create+delete for same tempId
		...
		for (const c of createItems) {
			const paired = deleteItems.find(
				(d) => d.tempId === c.tempId && d.id != null && c.id != null,
			);
		...
		for (const item of remaining) {
			...
			try {
				if (item.type === "create" && item.payload) {
					const result = await api.metrics.create(
						item.payload.encryptedData,
						item.payload.iv,
					);
					const realId = result.id;

					// Replace temp entry in IDB with real server entry
					...
					// Update any other queue items referencing tempId
					...
					await api.metrics.delete(item.serverId);
					if (item.id != null) await metricsStore.dequeue(item.id);
				}
			} catch {
				// Network failure — stop flush and mark as error; retry on next online event
				if (item.id != null) {
...
export const syncEngine = {
	init() {
		...
		// Handle background sync trigger from service worker
		...
		// Set initial status based on queue
