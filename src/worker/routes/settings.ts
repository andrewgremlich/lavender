...
settings.get('/', async (c) => {
  ...
  return c.json({ dataRetentionDays: row.data_retention_days } satisfies UserSettings);
});
...
settings.put('/', async (c) => {
  ...
  ).bind(userId, dataRetentionDays, dataRetentionDays).run();

  // Update expiration dates on existing entries
  const newExpiresAt = new Date(Date.now() + dataRetentionDays * 24 * 60 * 60 * 1000).toISOString();
  await c.env.DB.prepare(
    'UPDATE health_entries SET expires_at = ? WHERE user_id = ?'
  ).bind(newExpiresAt, userId).run();
