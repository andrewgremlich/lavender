...
metrics.get('/', async (c) => {
  ...
  ).bind(userId).all<HealthEntryRow>();
...
metrics.post('/', async (c) => {
  const userId = getUserId(c);
  const { encryptedData, iv } = await c.req.json<{ encryptedData: string; iv: string }>();
  ...
  ).bind(userId).first<{ data_retention_days: number }>();
  ...
  ).bind(id, userId, encryptedData, iv, expiresAt).run();
...
metrics.put('/:id', async (c) => {
  ...
  ).bind(encryptedData, iv, entryId, userId).run();
...
metrics.delete('/', async (c) => {
  ...
  await c.env.DB.prepare(
    'DELETE FROM health_entries WHERE user_id = ?'
  ).bind(userId).run();
