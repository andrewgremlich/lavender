...
function detectBBTOvulationDays(entries: EntryWithTemp[]): Set<string> {
	...
	for (let i = BASELINE_DAYS; i < valid.length; ) {
		...
		const requiredDays = isSlowRise ? SLOW_RISE_CONFIRMATION_DAYS : SUSTAINED_RISE_DAYS;
...
function analyzeCycleStats(periodStartDates: string[], ovulationDays: Set<string>): CycleStats {
	...
	if (cycleLengths.length >= 2) {
		...
		const variance =
			cycleLengths.reduce((sum, l) => sum + (l - mean) ** 2, 0) / (cycleLengths.length - 1);
	...
	if (sortedOvDates.length > 0 && periodStartDates.length >= 2) {
		...
		if (lutealLengths.length > 0) {
			lutealPhase = Math.round(lutealLengths.reduce((a, b) => a + b, 0) / lutealLengths.length);
		}
	}
...
function predictFutureCycles(
	...
	const extraDays =
		cycleVariability != null && cycleVariability > 2 ? Math.ceil(cycleVariability) : 0;
...
export function toFertilityEntry(
	...
	return {
		...rest,
		cervicalMucus: cervicalMucus ? (CM_SCORE_MAP[cervicalMucus] ?? undefined) : undefined,
...
export function getCycleDetails(entries: EntryWithTemp[]): CycleDetail[] {
	...
	return periodStartDates.map((periodStart, i) => {
		...
		const cycleLength = nextPeriodStart ? daysBetween(periodStart, nextPeriodStart) : undefined;
 ...
 */
export function getPredictionAccuracy(entries: EntryWithTemp[]): PredictionRecord[] {
	const sorted = [...entries].sort((a, b) => a.date.localeCompare(b.date));
	const { periodStartDates } = identifyPeriodDays(sorted);
...
export function calculateFertilityIndicators(entries: EntryWithTemp[]): FertilityIndicators {
	const sorted = [...entries].sort((a, b) => a.date.localeCompare(b.date));
	...
	const { periodDays, periodStartDates, averagePeriodLength } = identifyPeriodDays(sorted);

	const { averageCycleLength, cycleVariability, lutealPhase } = analyzeCycleStats(
		periodStartDates,
		ovulationDays
	);
	...
	const { predictedPeriodDays, predictedOvulationDays, predictedFertileDays } = predictFutureCycles(
		periodStartDates,
		cycleLengths,
		lutealPhase,
		cycleVariability,
