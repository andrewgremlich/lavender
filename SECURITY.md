...
- **Mitigation available:** Cap each field (e.g. `wrappedEncryptionKey` at 1 KB, `recoveryCodeHash` at 512 bytes, etc.) at all entry points.
- **Files:** `src/routes/api/auth/register/+server.ts:63-84`, `src/routes/api/auth/recovery-setup/+server.ts`, `src/routes/api/auth/recover/+server.ts`
...
### SQL string interpolation in seed scripts — **Low**
...
- **Rationale:** Scripts run locally on self-generated data. Not an attack surface unless the dev environment is already compromised.
...
- **Rationale:** Hardening. `X-Frame-Options: DENY` + existing HSTS already cover the main concerns.
